...
//function to get the complement
...
int getColumnIndex(char *table, char *column) { // table already .csv appended
    char tablecopy[200];
...
int selectComparator(struct AND_entry unit, char *str1, char* table_name) {
    if (unit.int1_fnd) {
        ...
        int colIndex = getColumnIndex(table_name, unit.col2);
        if (colIndex == -1) {
        ...
        int colIndex = getColumnIndex(table_name, unit.col1);
        if (colIndex == -1) {
        ...
        int colIndex1 = getColumnIndex(table_name, unit.col1);
        int colIndex2 = getColumnIndex(table_name, unit.col2);
        if (colIndex1 == -1) {
            printf("Column %s not found in table %s\n", unit.col1, table_name);
            return -1;
        }
...
int selectComputeCondition(struct OR_list condition, char *str, char* table_name){
    ...
    while(temp!=NULL){
        ...
        while(temp2!=NULL){
            ...
            int ret;
            if (temp2->is_cond == 1) {
                ret = selectComputeCondition(*(temp2->nest_condition), str_copy, table_name);
            }
            else {
                ret = selectComparator(*temp2, str_copy, table_name);
            }
            if(ret == -1){
                return -1;
            }
...
int equiComparator(struct AND_entry unit, char *str1, char *str2, char *table1, char *table2) {
    if (unit.int2_fnd) { // col op INT
        int colIndex = getColumnIndex(unit.table1, unit.col1);
        if (colIndex == -1) {
        ...
        int colIndex1 = getColumnIndex(unit.table1, unit.col1);
        int colIndex2 = getColumnIndex(unit.table2, unit.col2);
        if (colIndex1 == -1) {
            printf("Column %s not found in table %s\n", unit.col1, unit.table1);
            return -1;
        }
        ...
        char d1[50];
        sprintf(d1, "%s", getType(unit.table1, getColumnIndex(unit.table1, unit.col1)));
        if (strcmp(d1, "int") == 0) {
            if (strcmp(unit.table1, table1) == 0)
                unit.val1 = atoi(retval(str1, colIndex1));
            else
...
int equiComputeCondition(struct OR_list condition, char *str1, char *str2, char *table1, char *table2){
    AND_list* temp = condition.head;
    int result = 0;
    ...
    while(temp!=NULL){
        ...
        while(temp2!=NULL){
            ...
            int ret;
            if(temp2->is_cond == 1){
                ret = equiComputeCondition(*(temp2->nest_condition), str_copy1, str_copy2, table1, table2);
            } else {
                ret = equiComparator(*temp2, str_copy1, str_copy2, table1, table2);
            }
            if(ret == -1){
                return -1;
            }
            ...
            if (temp2->is_cond == 1) {
                ...
                if (temp2->int2_fnd) { // col op INT
                    if (temp2->table1 == NULL) {
                        if (getColumnIndex(table1, temp2->col1) != -1 && getColumnIndex(table2, temp2->col1) != -1) {
                            fprintf(stdout, "line no.: %d Column %s belongs to both %s and %s\n", yylinenumber, temp2->col1, table1, table2);
                            return -1;
                        }
                        else if (getColumnIndex(table1, temp2->col1) != -1) temp2->table1 = table1;
                        else if (getColumnIndex(table2, temp2->col1) != -1) temp2->table1 = table2;
                        else {
                        ...
                        else if (getColumnIndex(temp2->table1, temp2->col1) == -1) {
                            fprintf(stdout, "line no.: %d Column %s does not belong to %s\n",yylinenumber, temp2->col1, temp2->table1);
                            return -1;
                        }
                    }
                    char d1[50];
                    sprintf(d1, "%s", getType(temp2->table1, getColumnIndex(temp2->table1, temp2->col1)));
                    if (strcmp(d1, "int") != 0) {
                    ...
                    if (temp2->table1 == NULL) {
                        if (getColumnIndex(table1, temp2->col1) != -1) temp2->table1 = table1;
                        else if (getColumnIndex(table2, temp2->col1) != -1) temp2->table1 = table2;
                        else {
                            fprintf(stdout, "line no.: %d Column %s does not belong to %s or %s\n", yylinenumber,temp2->col1, table1, table2);
                            return -1;
                    ...
                    if (temp2->table2 == NULL) {
                        ...
                        if (getColumnIndex(table1, temp2->col2) != -1) temp2->table2 = table1;
                        else if (getColumnIndex(table2, temp2->col2) != -1) temp2->table2 = table2;
                        else {
                        ...
                        else if (getColumnIndex(temp2->table2, temp2->col2) == -1) {
                    ...
                    char d1[50], d2[50];
                    sprintf(d1, "%s", getType(temp2->table1, getColumnIndex(temp2->table1, temp2->col1)));
                    sprintf(d2, "%s", getType(temp2->table2, getColumnIndex(temp2->table2, temp2->col2)));
                    if (strcmp(d1, d2) != 0) {
...
int printEquiJoin(char *table1, char *table2, struct OR_list *conditions) {
    ...
    while(fgets(str1, 1000, file1)) {
        ...
        while (fgets(str2, 1000, file2)) {
            if (equiComputeCondition(*conditions, str1, str2, table1, table2)) {
                printf("%s", str1);
                printf(",%s", str2);
                numOfRecords++;
            };
